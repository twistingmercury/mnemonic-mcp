# Ralph Loop: Graph-Enhanced Pattern Search
...
| Item         | Path                                                              |
| ------------ | ----------------------------------------------------------------- |
| PRD          | `docs/plans/phase-03/PRD.md`                                     |
| Progress log | `docs/plans/phase-03/progress.txt`                                |
| Design spec  | `docs/superpowers/specs/2026-03-24-graph-enhanced-search-design.md` |
| Key context  | `src/mnemonic/internal/service/search/service.go`, `src/mnemonic/internal/repository/graph/graph.go`, `src/mnemonic/internal/repository/pattern/repository.go`, `src/mnemonic/internal/mcpserver/format.go`, `src/mnemonic/internal/server/server.go` |
...
9. Append one entry to `docs/plans/phase-03/progress.txt` (cycle name, outcome, any notable fixes).
...
- Full CI build (Docker image): `make build` — this is the final definition of done for the loop
...
**Step 5 — Delegate to the sub-agent.** Pass the cycle's Agent, Files, Steps, and Verify content. Add these baseline checks to every sub-agent task:
...
Exception: Cycle 6 uses `make build` as its final gate — baseline checks still apply to any source changes made during the cycle.

**Step 6 — Mark the PRD.** Change `- [ ]` to `- [x]` for the completed cycle.

**Step 7 — Update the progress log.** Append to `docs/plans/phase-03/progress.txt`:
...
- Do not modify the `search_patterns` MCP tool input schema — it is unchanged in this project.
- Do not refactor existing code unrelated to the current cycle.
