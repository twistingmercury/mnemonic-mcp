...
- [Data Flow: Agent Execution](#data-flow-agent-execution)
- [Data Flow: Pattern Updates](#data-flow-pattern-updates)
- [Scaling Characteristics](#scaling-characteristics)
- [Failure Modes](#failure-modes)
- [Security Boundaries](#security-boundaries)
- [That's the Architecture](#thats-the-architecture)
...
Clients["Clients"]
    ...
    subgraph K8s["Kubernetes Cluster"]
        ...
        subgraph MemoryPod["Shared Memory Service Pod"]
            MemoryApp["Shared Memory Service"]
        end
    ...
    subgraph Data["Data Layer"]
        ...
        MemoryDB[(PostgreSQL<br/>Shared Memory Data)]
...
- Calls the shared memory service for pattern queries
...
### Shared Memory Service Pod

Pattern search and knowledge graph queries happen here:

#### Shared Memory Service

- Official shared memory service Docker image (MCP server)
- Provides pattern search via MCP protocol
- Manages knowledge graph queries
- Handles vector embeddings
- Stores patterns in Neo4j

We're using the existing shared memory service MCP server directly via its native protocol. This keeps things simple and leverages the standard MCP interface.
...
sequenceDiagram
    ...
    API->>Memory: Search patterns (MCP)
    Memory->>Neo4j: Graph query
    Neo4j-->>Memory: Matching patterns
    Memory-->>API: Pattern results
...
**Pattern queries are on-demand** - Agent calls `search()` tool, we query the shared memory service, return just what's needed. This is the core efficiency gain.
...
sequenceDiagram
    ...
    CI->>Memory: POST /patterns/batch
    Memory->>Memory: Generate embeddings
    Memory->>Neo4j: Update knowledge graph
    Neo4j-->>Memory: Success
    Memory-->>CI: Patterns updated
    CI->>Dev: Notify (Slack)
```

Patterns are version controlled in git. When you commit, CI validates and loads them into the shared memory service. Next query gets the updated patterns. Simple.
...
#### Shared Memory PostgreSQL Dies

- Impact: Shared memory service pattern queries may fail
- Detection: Connection failures
- Recovery: RDS automatic failover to standby
- User impact: Agent executions fail with pattern query error
- Time to recover: < 5 minutes (RDS SLA)
- Mitigation: Shared memory service manages its own data; failures isolated from ACE app
...
- **Independent scaling** - API and shared memory service scale separately
