...
- [What We're NOT Building (Initially)](#what-were-not-building-initially)
- [Success Metrics](#success-metrics)
- [Critical Requirements](#critical-requirements)
- [Constraints We're Working With](#constraints-were-working-with)
...
**How we're doing it:**
Tool calling protocol. Agent calls a `search()` tool, we query the shared memory service's knowledge graph, return just the relevant patterns. See [ADR-006](02-architectural-decisions.md#adr-006-dynamic-pattern-querying) for the full rationale and cost analysis.
...
**How we're doing it:**
Patterns in git, loaded into the shared memory service instance. Update a pattern, commit, everyone gets it. Simple.
...
- Shared memory service scales for pattern query load
- Database resources allocated separately
- No forced coupling

**Why this matters:**
API requests and pattern searches have completely different resource profiles. API is CPU-bound and bursty. The shared memory service is memory-bound and steady. They shouldn't scale together.
...
**Shared Memory Service Integration**
Using existing shared memory service MCP server Docker images. Communication via standard MCP protocol. No modifications to the shared memory service itself. Support API evolution through versioning.
...
**Shared memory service multi-tenancy?**
Should we run one shared memory service instance for all teams or separate instances? Impact on resource isolation vs efficiency. We'll decide before production.
...
That's what we're building and why.
