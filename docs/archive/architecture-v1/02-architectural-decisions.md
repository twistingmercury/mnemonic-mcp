...
- [ADR-001: API-First Architecture](#adr-001-api-first-architecture)
- [ADR-002: REST External, gRPC Internal](#adr-002-rest-external-grpc-internal)
- [ADR-003: Shared Memory Service as Separate Service](#adr-003-shared-memory-service-as-separate-service)
- [ADR-004: Managed Databases](#adr-004-managed-databases)
- [ADR-005: Envoy + OPA for Auth/Authz](#adr-005-envoy--opa-for-authauthz)
- [ADR-006: Dynamic Pattern Querying](#adr-006-dynamic-pattern-querying)
- [ADR-007: Phased Development](#adr-007-phased-development)
...
**Option B: Hybrid (CLI + Optional Server)**  
...
4. **Better for high-frequency calls** - API -> shared memory service might happen 5-10 times per request. gRPC shines here.
...
## ADR-003: Shared Memory Service as Separate Service
...
The shared memory service handles our pattern search and knowledge graph queries. Should we embed it in the API server, run it as a separate service, or use an external SaaS?

### Decision

Deploy the shared memory service as a separate service in its own pod.
...
flowchart TB
    ...
    subgraph MemoryPod[Shared Memory Service Pod]
        MemoryMCP[Shared Memory Service Container]
    end
    APIPod -->|MCP Protocol| MemoryPod
```
...
1. **Independent lifecycle** - Update the shared memory service without restarting the API server. Deploy new versions independently.

2. **Resource isolation** - The shared memory service is memory-intensive (4-8GB). API server is CPU-bound (2-4GB). They have completely different resource profiles.

3. **Independent scaling** - Scale API (3-10 pods) and shared memory service (2-5 pods) based on their actual workload. API scales with request volume, shared memory service scales with pattern query load.

4. **Use existing MCP server** - The shared memory service ships as an MCP server Docker image. We communicate via standard MCP protocol.
...
Import the shared memory service as a library.
...
API and shared memory service containers in one pod.
...
**Option D: External SaaS:**
Use a hosted shared memory service.
...
- Can use official shared memory service MCP server
...
```mermaid
flowchart LR
    Agent[Agent needs pattern] --> Search[Calls search tool] --> Memory[Query Shared Memory] --> Return[Return relevant patterns]
```
...
Dynamic querying provides significant cost reduction compared to pre-loading all patterns into context. The savings come from reduced input token usage and more focused responses.

For current cost analysis with specific figures, verify against [Anthropic pricing](https://platform.claude.com/docs/en/about-claude/pricing).
...
- Requires shared memory service
...
Implement API server, shared memory service wrapper, CLI in parallel.
...
For detailed project timeline, see implementation plan.
...
Spending time on contracts upfront might seem slow, but it unlocks multiple teams working in parallel. The investment in contract definition pays off through parallel development efficiency.
...
- MCP protocol for shared memory service, not custom wrapper
...
That's the major architectural decisions and why we made them.
